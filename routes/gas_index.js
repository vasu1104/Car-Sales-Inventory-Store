...
router.get("/filter", async function (req, res) {
  ...
  if (priceBy != undefined) {
    ...
    console.log(priceLt);
    filtered_models = await GasModel.find({ price: { $lte: priceLt } }).sort({
      price: -1,
    });
  }
  ...
  if (yearLt != undefined) {
    ...
    console.log(yearLt);
    filtered_models = await GasModel.find({ year: { $lte: yearLt } }).sort({
      year: -1,
    });
  }
  ...
  if (mileageLt != undefined) {
    ...
    filtered_models = await GasModel.find({
      mileage: { $lte: mileageLt },
    }).sort({ mileage: -1 });
  }
